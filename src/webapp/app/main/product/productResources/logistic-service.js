define([], function () {
    ...
    function LogisticFactory($http) {
        ...
        function checkLogisticAvailability(pincode) {
            return $http.get(BY.config.constants.apiPrefix + BY.config.constants.productHost + '/logistic/pincode/' + pincode);
        }

        function trackOrderItem(awbList){
            if(awbList.length > 0){
                awbList = awbList.toString();
                return $http.get(BY.config.constants.apiPrefix + BY.config.constants.productHost + '/logistic/track?awb=' + awbList);
            }
        }
